...
def prepareTriangle(mesh: bmesh.types.BMesh) -> bmesh.types.BMesh:
    ...
    if (
        borderEdge is None
    ):  # If this condition is reached, the mesh is invalid (not two-manifold)
        raise RuntimeError(
            "Call to prepareTriangle with a closed mesh (not two-manifold mesh ?)."
        )
...
def merge(
    t0: bmesh.types.BMFace,
    t1: bmesh.types.BMFace,
...
def neighborsOf(
    triangle0: bmesh.types.BMFace, triangle1: bmesh.types.BMFace
) -> [bmesh.types.BMFace]:
    """
...
def commonEdgeOf(
    face1: [bmesh.types.BMFace], face2: [bmesh.types.BMFace]
...
def getShortestPathToTriangleFrom(
    mesh: bmesh.types.BMesh, triangle: bmesh.types.BMFace, mergeList, squarenessList
) -> [bmesh.types.BMFace]:
    """
    ...
    * shortest : the path with the smaller number of faces visited. Ties are
    ...
    print(paths)
    raise RuntimeError(
        "Breadth first visit failed to find another reachable triangle (not connected mesh ?)."
    )
...
def triToQuad(mesh: bmesh.types.BMesh) -> bmesh.types.BMesh:
    ...
    optimalEdges = {
        face: min(face.edges, key=lambda x: squarenessList[x]) for face in mesh.faces
    }
    ...
    for face in mesh.faces:
        ...
        if (
            mergeList[otherFace] is not None
        ):  # Optimal link of the current face is already linked
            continue

        face_score = squarenessList[optimalEdge]
        otherFace_score = squarenessList[optimalEdges[otherFace]]
        if (
            face_score > otherFace_score
        ):  # Optimal link of the current face has a better link (= smaller score)
            continue
    ...
    for face in mesh.faces:
        if (
            merged[face] or mergeList[face] is None
        ):  # Face i is already merged or unmergeable
            continue
